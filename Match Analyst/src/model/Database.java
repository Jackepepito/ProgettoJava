...
public class Database {
	...
	public void testInsert() {
        ...
        st.executeUpdate("CREATE TABLE IF NOT EXISTS giocatore (numero INTEGER primary key, squadra varchar(25) references utente(squadra), nome VARCHAR(50), ruolo varchar(20), gol INTEGER,golsubiti INTEGER)");
        st.executeUpdate("CREATE TABLE IF NOT EXISTS partita (avversario VARCHAR(50), squadra varchar(25) references utente(squadra), casa_trasferta varchar(20), gol_segnati INTEGER, gol_subiti INTEGER, "
        		+ "possesso_palla INTEGER, tiri_tot INTEGER ,tiri_porta INTEGER, falli_commessi INTEGER, falli_subiti INTEGER, parate INTEGER, PRIMARY KEY (avversario, squadra, casa_trasferta))");
        st.executeUpdate("CREATE TABLE IF NOT EXISTS marcatore (partita varchar(50) references partita(avversario), casa_trasferta varchar(20) references partita(casa_trasferta), giocatore VARCHAR(50) references giocatore(nome), gol INTEGER, PRIMARY KEY (partita, squadra, casa_trasferta))");
        }
		catch (SQLException e){ 
			e.printStackTrace();
		}
